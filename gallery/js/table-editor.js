    ...
    TableEditor.prototype = {
        ...
        addRow: function (rowData, rows) {
            ...
            for (; i < rowsToAdd; i++) {
                ...
                for (j = 0; j <= cellsNumber; j++) {
                    ...
                    td.innerHTML = rowData[j] ? (typeof rowData[j] === 'string' ? rowData[j] : parseInt(rowData[j])) : '';
                    console.log(typeof rowData[j] === 'string');
                    tr.setAttribute('data-row-index', rowData[0]);
        ...
        editContentHandler: function () {
            ...
            content.addEventListener('click', function (e) {
                ...
                modelEntryNumber = self.model.getRowByID(target.parentNode.getAttribute('data-row-index') - 1);
                ...
                target.addEventListener('blur', function () {
                    if (target.innerHTML !== modelEntryNumber[this.getAttribute('data-index')]) {
                        console.log(typeof target.innerHTML);
                        modelEntryNumber[this.getAttribute('data-index')] = target.innerHTML;
                    }
                });
                return false;
            });
